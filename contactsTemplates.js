function printContactAlphabethicContainer(sortLetterNr) {
    contactCarts.innerHTML += `
    <div class="contactSort">
        <p>
            ${sortLetterNr['letter']}
        </p>
    </div>
`;
}


function printContact(LetterContactNr) {
    let color = LetterContactNr['color'];
    let initials = LetterContactNr['name'].match(/\b(\w)/g).join('');
    let name = LetterContactNr['name'];
    let email = LetterContactNr['email'];
    let phone = LetterContactNr['phone'];

    contactCarts.innerHTML += `
        <div class="contactCart" onclick="showContactDetails('${color}', '${initials}', '${name}', '${email}', '${phone}')" tabindex="1">
            <div class="initialsContactCart" style="background: ${color}">
                <p id="contactCartInitals">${initials}</p>
            </div>
            <div class="nameAndEmail">
                <p class="contactCartName" id="contactCartName">
                    ${name}
                </p>
            <span type="email" id="contactCartEmail" style="color: #29ABE2;">${email}</span>
            </div>
        </div>
    `;
}


function showContactDetails(color, initials, name, email, phone) {
    let infoBox = document.getElementById('contactsRightsideContentInfoBox');
    infoBox.classList.add('contactsRightsideContentInfoBoxTransition');
    infoBox.innerHTML = `
        <div class="contactsRightsideContentInfoBoxProfile">
            <p class="contactsRightsideContentInfoBoxProfileInitials" style="background: ${color};">
            ${initials}
            </p>
            <div class="contactsRightsideContentInfoBoxProfileName">
                <h1>${name}</h1>
                <div class="contactsRightsideContentInfoBoxProfileButtons">
                    <button onclick="showEditContact('addContactPopupBackground', '${color}', '${initials}', '${name}', '${email}', '${phone}')"><svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M2 17H3.4L12.025 8.375L10.625 6.975L2 15.6V17ZM16.3 6.925L12.05 2.725L13.45 1.325C13.8333 0.941667 14.3042 0.75 14.8625 0.75C15.4208 0.75 15.8917 0.941667 16.275 1.325L17.675 2.725C18.0583 3.10833 18.2583 3.57083 18.275 4.1125C18.2917 4.65417 18.1083 5.11667 17.725 5.5L16.3 6.925ZM14.85 8.4L4.25 19H0V14.75L10.6 4.15L14.85 8.4Z" fill="#2A3647"/>
                    </svg>Edit</button>
                    <button onclick="deleteContact('${name}')"><svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M3 18C2.45 18 1.97917 17.8042 1.5875 17.4125C1.19583 17.0208 1 16.55 1 16V3C0.716667 3 0.479167 2.90417 0.2875 2.7125C0.0958333 2.52083 0 2.28333 0 2C0 1.71667 0.0958333 1.47917 0.2875 1.2875C0.479167 1.09583 0.716667 1 1 1H5C5 0.716667 5.09583 0.479167 5.2875 0.2875C5.47917 0.0958333 5.71667 0 6 0H10C10.2833 0 10.5208 0.0958333 10.7125 0.2875C10.9042 0.479167 11 0.716667 11 1H15C15.2833 1 15.5208 1.09583 15.7125 1.2875C15.9042 1.47917 16 1.71667 16 2C16 2.28333 15.9042 2.52083 15.7125 2.7125C15.5208 2.90417 15.2833 3 15 3V16C15 16.55 14.8042 17.0208 14.4125 17.4125C14.0208 17.8042 13.55 18 13 18H3ZM3 3V16H13V3H3ZM5 13C5 13.2833 5.09583 13.5208 5.2875 13.7125C5.47917 13.9042 5.71667 14 6 14C6.28333 14 6.52083 13.9042 6.7125 13.7125C6.90417 13.5208 7 13.2833 7 13V6C7 5.71667 6.90417 5.47917 6.7125 5.2875C6.52083 5.09583 6.28333 5 6 5C5.71667 5 5.47917 5.09583 5.2875 5.2875C5.09583 5.47917 5 5.71667 5 6V13ZM9 13C9 13.2833 9.09583 13.5208 9.2875 13.7125C9.47917 13.9042 9.71667 14 10 14C10.2833 14 10.5208 13.9042 10.7125 13.7125C10.9042 13.5208 11 13.2833 11 13V6C11 5.71667 10.9042 5.47917 10.7125 5.2875C10.5208 5.09583 10.2833 5 10 5C9.71667 5 9.47917 5.09583 9.2875 5.2875C9.09583 5.47917 9 5.71667 9 6V13Z" fill="#2A3647"/>
                    </svg>Delete</button>
                </div>
            </div>
         </div>
        <div class="contactsRightsideContentInfoBoxContactInformationBox">
            <div class="contactsRightsideContentInfoBoxContactInformationHeader">
                <p>Contact Information</p>
            </div>
            <div>
                <p class="bigText">Email</p>
                <a href="mailto:${email}" style="color: #29ABE2;">${email}</a>
            </div>
            <div>
                <p class="bigText">Phone</p>
                <a href="tel:${phone}">${phone}</a>
            </div>
            
        </div>
        <div class="moreDropdownContainer" onclick="showMobileOptions('dropdownContainerContent')">
            <img src="./img/more_vert.svg">
        </div>
        <div id="dropdownContainerContent" class="dropdownContainerContent vanish">
            <div class="dropdownContainerContentContainer">
                <button onclick="showEditContact('addContactPopupBackground', '${color}', '${initials}', '${name}', '${email}', '${phone}')"><svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M2 17H3.4L12.025 8.375L10.625 6.975L2 15.6V17ZM16.3 6.925L12.05 2.725L13.45 1.325C13.8333 0.941667 14.3042 0.75 14.8625 0.75C15.4208 0.75 15.8917 0.941667 16.275 1.325L17.675 2.725C18.0583 3.10833 18.2583 3.57083 18.275 4.1125C18.2917 4.65417 18.1083 5.11667 17.725 5.5L16.3 6.925ZM14.85 8.4L4.25 19H0V14.75L10.6 4.15L14.85 8.4Z" fill="#2A3647"/>
                </svg>Edit</button>
                <button onclick="deleteContact('${name}')"><svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M3 18C2.45 18 1.97917 17.8042 1.5875 17.4125C1.19583 17.0208 1 16.55 1 16V3C0.716667 3 0.479167 2.90417 0.2875 2.7125C0.0958333 2.52083 0 2.28333 0 2C0 1.71667 0.0958333 1.47917 0.2875 1.2875C0.479167 1.09583 0.716667 1 1 1H5C5 0.716667 5.09583 0.479167 5.2875 0.2875C5.47917 0.0958333 5.71667 0 6 0H10C10.2833 0 10.5208 0.0958333 10.7125 0.2875C10.9042 0.479167 11 0.716667 11 1H15C15.2833 1 15.5208 1.09583 15.7125 1.2875C15.9042 1.47917 16 1.71667 16 2C16 2.28333 15.9042 2.52083 15.7125 2.7125C15.5208 2.90417 15.2833 3 15 3V16C15 16.55 14.8042 17.0208 14.4125 17.4125C14.0208 17.8042 13.55 18 13 18H3ZM3 3V16H13V3H3ZM5 13C5 13.2833 5.09583 13.5208 5.2875 13.7125C5.47917 13.9042 5.71667 14 6 14C6.28333 14 6.52083 13.9042 6.7125 13.7125C6.90417 13.5208 7 13.2833 7 13V6C7 5.71667 6.90417 5.47917 6.7125 5.2875C6.52083 5.09583 6.28333 5 6 5C5.71667 5 5.47917 5.09583 5.2875 5.2875C5.09583 5.47917 5 5.71667 5 6V13ZM9 13C9 13.2833 9.09583 13.5208 9.2875 13.7125C9.47917 13.9042 9.71667 14 10 14C10.2833 14 10.5208 13.9042 10.7125 13.7125C10.9042 13.5208 11 13.2833 11 13V6C11 5.71667 10.9042 5.47917 10.7125 5.2875C10.5208 5.09583 10.2833 5 10 5C9.71667 5 9.47917 5.09583 9.2875 5.2875C9.09583 5.47917 9 5.71667 9 6V13Z" fill="#2A3647"/>
                </svg>Delete</button>
            </div>
        </div>
    `;
}


function editCart(color, initials, name, email, phone) {
    let popup = document.getElementById('addContactPopup');
    popup.innerHTML = `
        <div class="addContactCardLeftSide">
            <div class="addContactCardLeftSideContent">
                <img src="./img/Capa-2.svg">
                <h1>Edit contact</h1>
                <div class="addContactCardLeftSideBreake">

                </div>
            </div>
        </div>
        <div class="addContactCardRightSide">
            <div class="addContactCardRightSideContent">
                <div class="addContactCardRightSideContentImg">
                    <p class="contactsEditContactInitials" style="background: ${color};">
            ${initials}
            </p>
                </div>
                <div class="addContactCardRightSideContentInputfieldsAndButtons">

                    <div class="addContactCardRightSideContentInputfields">
                        <div class="closeButtonDiv">
                            <div class="closeButtonBackground"
                                onclick="vanishAddConact()">
                                <img class="addContactCloseButton" src="./img/close.svg">
                            </div>
                        </div>
                         <form id="editContactInputForm">
                            <div class="input">
                                <input id="contactName" class="addContactInput" placeholder="Name"
                                    type="text"
                                    title="Please enter at least two words with initial capital letters"
                                    value="${name}" >
                                <img src="./img/person.svg">
                            </div>
                            <div class="input">
                                <input id="contactEmail" class="addContactInput" placeholder="Email"
                                    type="email" value="${email}">
                                <img src="./img/mail.svg">
                            </div>
                             <div class="input">
                                <input id="contactPhone" class="addContactInput" placeholder="Phone"
                                    type="tel" pattern="([0-9])+" title="Only Nummbers allowed" value="${phone}">
                                <img src="./img/call.svg">
                            </div>
                            <div class="addContactButtons">
                                <button type="reset" class="cancelButton"
                                     onclick="deleteContact('${name}'), vanishAddConact()">Delete
                                </button>
                                 <button type="submit" class="editContactButton">Save <img src="./img/buttonCheck.svg">
                                 </button>
                            </div>
                        </form>

                    </div>

                </div>
            </div>
        </div>
    `;
}


function addNewContactCart() {
    let popup = document.getElementById('addContactPopup');
    popup.innerHTML = `
        <div class="addContactCardLeftSide">
            <div class="addContactCardLeftSideContent">
                <img class="addContactCardLeftSideContentImg" src="./img/Capa-2.svg">
                <h1>Add contact</h1>
                <p>Tasks are better with a Team!</p>
                <div class="addContactCardLeftSideBreake">

                </div>
                <div onclick="vanishAddConact()">
                    <img class="closeButtonMobile" src="./img/close_white.svg">
                </div>
            </div>
        </div>
        <div class="addContactCardRightSide">
            <div class="addContactCardRightSideContent">
                <div class="addContactCardRightSideContentImg">
                    <img src="./img/person_white.svg">
                </div>
                <div class="addContactCardRightSideContentInputfieldsAndButtons">

                    <div class="addContactCardRightSideContentInputfields">
                        <div class="closeButtonDiv">
                            <div class="closeButtonBackground"
                                onclick="vanishAddConact()">
                                <img class="addContactCloseButton" src="./img/close.svg">
                            </div>
                        </div>
                        <form id="addContactInputForm">
                            <div class="input">
                                <input id="contactName" class="addContactInput" placeholder="Name"
                                    type="text" required"
                                    title="Please enter at least two words with initial capital letters"
                                    required>
                                <img src="./img/person.svg">
                            </div>
                            <div class="input">
                                <input id="contactEmail" class="addContactInput" placeholder="Email"
                                    type="email" required>
                                <img src="./img/mail.svg">
                            </div>
                            <div class="input">
                                <input id="contactPhone" class="addContactInput" placeholder="Phone"
                                    type="tel" pattern="([0-9])+" title="Only Nummbers allowed" required>
                                <img src="./img/call.svg">
                            </div>
                            <div class="addContactButtons">
                                <button type="reset" class="cancelButton"
                                    onclick="vanishAddConact()">Cancel
                                    <svg class="iconSVG" width="14" height="14" viewBox="0 0 14 14"
                                         fill="#2A3647">
                                        <path
                                            d="M7.06917 7.00008L12.3122 12.2431M1.82617 12.2431L7.06917 7.00008L1.82617 12.2431ZM12.3122 1.75708L7.06817 7.00008L12.3122 1.75708ZM7.06817 7.00008L1.82617 1.75708L7.06817 7.00008Z"
                                            stroke="#2A3647" stroke-width="2" stroke-linecap="round"
                                            stroke-linejoin="round" />
                                    </svg>
                                </button>
                                <button type="submit" class="createContactButton">
                                Create contact <img src="./img/buttonCheck.svg">
                                </button>
                            </div>
                        </form>

                    </div>

                 </div>
            </div>
        </div>
    `;
}